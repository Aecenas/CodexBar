<div align="center">
  ...
  <p>
    <img alt="Version" src="https://img.shields.io/badge/version-1.1.0-7ef6c7?style=for-the-badge" />
...
CodexBar 是一个 Windows 托盘式桌面小工具。它读取本机 Codex 的额度与 session 写入活动，把 5 小时窗口、一周窗口、忙碌状态、历史曲线、token 用量和运行设置压进一条透明顶栏里。
...
- **活跃状态闪电**：检测到本机 Codex session 正在写入时，左侧闪电图标会发光闪动。
...
| 闪电图标 | 本机 Codex session 近期写入时发光。 |
| 不透明 Bar 区域 | 开启位置调整后可拖动；右键可打开自绘退出菜单。 |
| 透明区域 | 鼠标穿透到底层窗口。 |

### 设置项

- `状态检查`：检查本机 Codex session 写入活动的间隔，默认 10 秒，下限 5 秒。
...
`代理与诊断` 里的下载代理是一个 **HTTPS URL 前缀**。CodexBar 会把 GitHub Release 安装包的完整下载地址拼在它后面，并在执行前校验 GitHub 提供的 SHA-256 摘要和资产大小。
...
- Node.js 20 或更新版本
- npm
- 本机已安装并登录 Codex CLI，且 `codex` 命令可从 `PATH` 运行
...
4. `Release` workflow 会在 Windows runner 上执行 `npm ci`、`npm test` 和 `npm run dist`。
5. 生成的 `.exe`、`.blockmap` 和 `latest.yml` 会上传到对应 GitHub Release。

### FAQ

**为什么闪电没有亮？**  
CodexBar 监视本机 session JSONL 的写入活动；如果当前任务还没有产生新的 session 写入，闪电可能暂时保持空闲。可以在 `代理与诊断` 里查看最近命中来源。
...
CodexBar is a Windows tray utility that turns local Codex quota, session write activity, quota history, token usage, and runtime controls into a compact transparent desktop bar.
...
- **Busy lightning effect**: the bolt glows while local Codex session files are being written.
...
| Bolt icon | Glows when recent local Codex session writes are detected. |
...
- `状态检查`: interval for checking local Codex session write activity. Default 10s, minimum 5s.
...
The download proxy is an **HTTPS URL prefix** used only for GitHub Release installer downloads. CodexBar verifies the GitHub-provided SHA-256 digest and asset size before launching the installer.
...
- Node.js 20 or newer
- npm
- Codex CLI installed and authenticated, with the `codex` command available on `PATH`
...
4. The `Release` workflow runs `npm ci`, `npm test`, and `npm run dist` on a Windows runner.
5. The generated `.exe`, `.blockmap`, and `latest.yml` files are uploaded to the GitHub Release.

### FAQ

**Why is the bolt not glowing?**  
CodexBar watches local session JSONL writes. If the current task has not produced a new session write yet, the bolt may remain idle temporarily. The diagnostics panel shows the most recent source.
